...
static void generateXMLForMember(const MemberDef *md,FTextStream &ti,FTextStream &t,const Definition *def)
{
     ...
     )
  {
    ...
    t << "</type>" << endl;
    QCString DefStr = convertToXML(md->definition());
    printf("[yz] raw def=%s, conv def=%s\n", md->definition(), DefStr.data());
    if (DefStr.find("STRU_Test_5::@1::value") != -1 )
    {
        printf("");
    }
...
static void generateXMLForClass(const ClassDef *cd,FTextStream &ti)
{
  ...
  /*FIXME [yangzheng] 2019/10/21 
   * 需要修复匿名内联复合对象的collaborationGraph不输出的问题
   */
...
void generateXML()
{
...

  {
    ...
    printf("[yz] Doxygen::classSDict count=%d\n", Doxygen::classSDict->count());
    const ClassDef *cd;
    for (cli.toFirst();(cd=cli.current());++cli)
    {
        printf("[yz] class=%s\n", cd->className().data());
