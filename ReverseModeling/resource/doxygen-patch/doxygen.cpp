...
{
    printf("[yz] addVariableToClass cd->name=%s\n", cd->name().data());
  // FIXME 20191021 排查错误时，定位到这个接口，这个接口调用前，解析复合对象的结构是正确的，
  // 作用于也正确，匿名对象也正确，从这个接口开始，处理就不太对了，猜测应该是被接口在调用addMemberDocs时，
  // 没有按照所属域添加成员，将内联对象的成员添加父对象了
    QCString qualScope;
  if (fromAnnScope)
  {
      qualScope = scope;
  ...
  {
      qualScope = cd->qualifiedNameWithTemplateParameters();
  }
  ...
  if (!type.isEmpty())
  {
    ...
    else
    {
      ...
      {
          std::string strQualScope = qualScope.data();
  ...
  /*FIXED [yangzheng] 2019/10/21
  * 此处def有点问题，匿名对象应该加上作用域
  */
  printf("[yz] setDef=%s\n", def.data());
 ...
 * @fn addVariable
 * @brief 从语法树中解析一个变量
 * @param root [in] 语法树中一个节点
 * @param isFuncPtr [in] 标记是否是函数指针
 * @return None.
 * 
 * @fixme 此接口在处理匿名内联对象有问题
 */
static void addVariable(const Entry *root,int isFuncPtr=-1)
{
    ...
    if (cd)
    {
      ...
      if (si!=-1 && !inlineSimpleStructs) // anonymous scope or type
      {
        ...
        printf("[yz] scope=%s pScop=%s annScopePrefix=%s\n", scope.data(), pScope.data(), annScopePrefix.data());
        if (!pScope.isEmpty())
        ...
        if (name.at(0)!='@')
        {
            /*FIXME [yangzheng] 2019/10/21
            * 此处有个bug，name为变量名，从代码的逻辑来看，坐着是想通过判断名字中是否带'@'符号来判断
            * 变量是不是属于某个匿名内敛对象，然而在代码的这一层，name中是不带'@'的，真正带'@'符号的是 scope 字段，
            * 于是增加如下代码，addVariableToClass的第二个参数应该是用scope搜索得到的class
            */
            if (scope.find('@') != -1)
            {
                pScope = scope;
          ...
          {
            // [add] by yangzheng 2019/10/21 
              printf("[yz] (anonymous) <1> type=%s name='%s' scope=%s scope.right=%s\n",
                  type.data(),
                  name.data(), scope.data(),
                  scope.right(scope.length() - si).data()
              );
              printf("[yz] pcd.classname=%s pScop=%s\n", pcd->className().data(), pScope.data());
      ...
      // [add] by yangzheng 2019/10/21, 向ClassDef对象增加成员时，添加打印
      printf("[yz] <2> type=%s name='%s' scope=%s scope.right=%s\n",
          type.data(),
          name.data(), scope.data(),
          scope.right(scope.length() - si).data());
      addVariableToClass(root,   // entry
