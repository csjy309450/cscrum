...
{
  // FIXME 20191021 应该增加匿名内敛对象的处理
    if (md->fromAnonymousScope())
    {
        MemberDef * pAnonMd = md->fromAnonymousMember();
        ClassDef * pAnonClass = md->getClassDefOfAnonymousType();
        printf("");
    }
            ...
            if (md->isVariable())
            {
              switch (prot)
              {
                ...
                case Package:
                  ...
                  break;
                case Public:
                    printf("[yz] addMemberToList name=%s\n", md->name().data());
         ...
         QCString(md->typeString())=="friend union")))
  {
    ...
    printf("[yz] allMemberNameInfoSDict count=%d\n", m_impl->allMemberNameInfoSDict->count());
    /*FIXME [yangzheng] 2019/10/21 
     * 通过前后断点，发现匿名内敛对象重名成员丢失问题，由此处引发
     * 现象：
     * struct STRU_Test_5 {
          int value;
          struct {
     ...
     * 此种情况下，3个value对象只会记录最后一个，前面的被丢失
     */
    ...
    QList<ArgumentList> *actualParams,int *actualParamIndex) const
{
  ...
  scName+=clName; /* [yangzheng] 此处只加上了类名，如果是匿名内联对象的成员还应该加上完整作用域名*/
  ArgumentList *al=0;
  if (templateArguments())
  {
 ...
 * @fn addMemberToList
 * @brief 
 * @param lt [in] ClassDef对象包含多个MemberList，包括私有成员、共有成员等等，lt标识成员应该加入哪个列表
 * @return 返回说明
 */
void ClassDefImpl::addMemberToList(MemberListType lt,MemberDef *md,bool isBrief)
{
  //FIXME [20191021] 这个接口每个成员变量调用两次，需要确认什么情况，另外MemberList总感觉不对，createMemberList里面都干什么了？
  static bool sortBriefDocs = Config_getBool(SORT_BRIEF_DOCS);
  static bool sortMemberDocs = Config_getBool(SORT_MEMBER_DOCS);
  MemberList *ml = createMemberList(lt);
  ml->setNeedsSorting((isBrief && sortBriefDocs) || (!isBrief && sortMemberDocs));
  printf("[yz] MemberList %d pre count=%d\n", lt, ml->count());
  ml->append(md);
  printf("[yz] MemberList %d post count=%d\n", lt, ml->count());

  // [add] by yangzheng 2019/10/21
  printf("[yz] type=%s, name=%s\n", md->typeString(), md->name().data());
  ClassDef * pClass = md->category();
  //printf("[yz] category class type=%s, name=%s\n", pClass->className().data());
